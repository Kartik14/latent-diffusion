...
class MammalsBase(Dataset):
    def __init__(
        ...
        # assign and idx to each class
        self.class_idx = {c: i for i, c in enumerate(set(self.labels["human_label"]))}
        print(self.class_idx)
        self.labels["class_label"] = [
            self.class_idx[c] for c in self.labels["human_label"]
        ]
        self.size = size
        ...
        for idx in trange(len(self.labels["file_path_"])):
            # for idx in trange(1000):
            label = dict((k, self.labels[k][idx]) for k in self.labels)
            path = label["file_path_"]
...
class MammalsTrain(MammalsBase):
    def __init__(self, **kwargs):
        super().__init__(
            txt_file="data/mammals/train.txt",
...
class MammalsVal(MammalsBase):
    def __init__(self, flip_p=0.0, **kwargs):
        super().__init__(
            txt_file="data/mammals/val.txt",
            data_root="data/mammals",
            flip_p=flip_p,
            **kwargs,
        )
